...
async function addItem(url, item) {
  try {
    ...
    if (res.status !== 201) throw new Error("Fail to add item")  //จัดการ error mode 400,500
...
async function deleteItem(url, id) {
  try {
    ...
    if(!res.ok) throw new Error("Fail to delete item") //อันนี้เขียนแบบย่อ ถ้าอยากดูฉบับเต็มให้ไปดู GET ข้างบน
    return id
  } catch (error) {
    throw new Error(error.message)
  }
